...
SELECT
    ...
    -- Business Logic: Clinical Classification Brackets
    CASE 
        -- Catch the massive pile of blanks first
        WHEN vh.primary_diagnosis_description IS NULL THEN 'Administrative / No Diagnosis Provided'
        
        -- Catch Social Determinants and Employment Status
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%employment%' 
        ...
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%diabetes%' 
        ...
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%pneumonia%' 
        ...
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%infection%' 
        ...
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%depress%' 
        ...
        -- Physical trauma keywords
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%injury%' 
        ...
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%arthritis%' 
        ...
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%neoplasm%' 
        ...
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%pregnancy%' 
          OR LOWER(vh.primary_diagnosis_description) LIKE '%maternal%' THEN 'Maternal & Pregnancy'
          
        WHEN LOWER(vh.primary_diagnosis_description) LIKE '%medication%' 
    ...
    DATE_DIFF(CAST(vh.encounter_start_datetime AS DATE), CAST(vh.previous_encounter_start AS DATE), DAY) AS days_since_last_visit,
    
    -- The Business Logic Flag
    CASE 
        WHEN DATE_DIFF(CAST(vh.encounter_start_datetime AS DATE), CAST(vh.previous_encounter_start AS DATE), DAY) <= 30 
